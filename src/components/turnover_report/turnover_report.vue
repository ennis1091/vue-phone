      ...
      >
        ...
        <div
          ...
          :style="{
            background:
              'url(' +
              require('@/common/img/turnover-right.png') +
              ') no-repeat center center',
            'background-size': '100% 100%'
          }"
        >
    ...
    <instructions :imgURL="imgURL" :text="text" v-show="on" @closed="closed">
      <ul class="alert-ul">
        <li>
          ...
          <div><span>结算方式：</span>不结算、实时结算、T+n结算、M+1结算。</div>
        </li>
        <li>
          <div><i class="v-i"></i></div>
          <div><span>不结算：</span>不进行结算转账处理。</div>
        </li>
        <li>
          <div><i class="v-i"></i></div>
          <div><span>实时结算：</span>系统自动进行结算转账处理。</div>
        </li>
        <li>
          <div><i class="v-i"></i></div>
          <div>
            <span>T+1结算：</span
            >T日为交易日（含节假日），T日交易完成的订单将于一天后结算。
          </div>
        </li>
        <li>
          <div><i class="v-i"></i></div>
          <div>
            <span>T+7结算：</span
            >T日为交易日（含节假日），T日交易完成的订单将于下周一凌晨结算。
          </div>
        </li>
        <li>
          <div><i class="v-i"></i></div>
        ...
        <li>
          ...
          <div>
            <span
              >未满足最低结算金额（￥1.00）或未结算订单
              将顺延至下一结算周期（含节假日）。</span
            >
          </div>
        </li>
      </ul>
...
export default {
  data () {
      ...
      this.chartEvents = {
        click: function (e) {
          self.name = e.name
          self.$router.push({name: 'orderlist', query: { plan: '123' }})
        }
      },
      this.extend ={
          xAxis: {
